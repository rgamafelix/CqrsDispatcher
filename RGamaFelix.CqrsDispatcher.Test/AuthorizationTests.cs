...
public class AuthorizationTests
{
  ...
  public async Task CommandHandlerExtensionShouldCallNextWhenPolicySucceeds()
  {
    ...
    var sut = new AuthorizationCommandHandlerExtension<AuthorizedCommandHandler, TestCommandRequest>(accessor,
      authService, NullLogger<ICommandHandlerExtension<AuthorizedCommandHandler, TestCommandRequest>>.Instance);
  ...
  public void CommandHandlerExtensionShouldSkipWhenHandlerHasNoAuthorizationAttribute()
  {
    ...
    var sut = new AuthorizationCommandHandlerExtension<UnauthorizedCommandHandler, TestCommandRequest>(accessor,
      authService, NullLogger<ICommandHandlerExtension<UnauthorizedCommandHandler, TestCommandRequest>>.Instance);
  ...
  public async Task CommandHandlerExtensionShouldThrowWhenNoHttpContext()
  {
    ...
    // Act & Assert
    await Assert.ThrowsAsync<UnauthorizedRequestException>(() => sut.Handle(new TestCommandRequest(),
      new AuthorizedCommandHandler(), (_, _) => Task.CompletedTask, CancellationToken.None));
  }
  ...
  public async Task QueryHandlerExtensionShouldThrowWhenNoHttpContext()
  {
    ...
    // Act & Assert
    await Assert.ThrowsAsync<UnauthorizedRequestException>(() => sut.Handle(new TestQueryRequest(),
      new AuthorizedQueryHandler(), (_, _) => Task.FromResult(new TestQueryResponse()), CancellationToken.None));
  }
